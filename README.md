...
| validator_count_with_sync_participation_less_avg                          | nos_name                         | Number of validators with sync committee participation less than avg for each user Node Operator                                                                                                    |
...
| validator_count_invalid_attestation_last_n_epoch                          | nos_name, reason, epoch_interval | Number of validators with invalid properties (head, target, source) \ high inc. delay in attestation in the last `BAD_ATTESTATION_EPOCHS` epochs for each user Node Operator                        |
...
| validator_count_high_inc_delay_last_n_epoch                               | nos_name, epoch_interval         | Number of validators with inc. delay > 2 in the last N epochs for each user Node Operator                                                                                                           |
| validator_count_invalid_attestation_property_last_n_epoch                 | nos_name, epoch_interval         | Number of validators with two invalid attestation property (head or target or source) in the last N epochs for each user Node Operator                                                              |
...
| validator_count_with_sync_participation_less_avg_last_n_epoch             | nos_name, epoch_interval         | Number of validators with sync participation less than avg in the last `SYNC_PARTICIPATION_EPOCHS_LESS_THAN_CHAIN_AVG` epochs for each user Node Operator                                           |
| high_reward_validator_count_with_sync_participation_less_avg_last_n_epoch | nos_name, epoch_interval         | Number of validators with sync participation less than avg in the last `SYNC_PARTICIPATION_EPOCHS_LESS_THAN_CHAIN_AVG` epochs (with possible high reward in the future) for each user Node Operator |
