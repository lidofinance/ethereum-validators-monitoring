...
export class CriticalMissedAttestations extends Alert {
  ...
  constructor(env: Environment, storage: ClickhouseStorage) {
    super(CriticalMissedAttestations.name, env, storage);
  }
  ...
  async alertRule(bySlot: bigint): Promise<AlertRuleResult> {
    ...
    const missedAttValidatorsCount = await this.storage.getValidatorCountWithMissedAttestationsLastNEpoch(
      bySlot, this.env.BAD_ATTESTATION_EPOCHS
    );
  ...
  alertBody(ruleResult: AlertRuleResult): AlertRequestBody {
    return {
      ...
      annotations: {
        summary: `${Object.values(ruleResult).length} Node Operators with CRITICAL count of validators with missed attestations in the last ${this.env.BAD_ATTESTATION_EPOCHS} epoch`,
        description: join(Object.entries(ruleResult).map(([o, r]) => `${o}: ${r.missedAtt} of ${r.ongoing}`), '\n'),
      }
    }
  };
}
