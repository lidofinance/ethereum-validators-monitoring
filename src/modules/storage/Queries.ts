...
export const validatorsCountWithSyncParticipationLessChainAvgLastNEpochQuery = (
  slot: string | bigint | number, fetchInterval: number, epochInterval: number, distance: number, validatorIndexes: string[] = []
): string => {
...
export const validatorCountWithMissAttestationLastNEpochQuery = (
  fetchInterval: number, slot: string, epochInterval: number, validatorIndexes: string[] = []
): string => {
  let strFilterValIndexes = '';
  if (validatorIndexes.length > 0) {
    strFilterValIndexes = `AND validator_id in [${validatorIndexes.map(i => `'${i}'`).join(',')}]`;
...
export const validatorsCountWithMissProposeQuery = (
  fetchInterval: number, slot: string, validatorIndexes: string[] = []
): string => {
  let strFilterValIndexes = '';
  if (validatorIndexes.length > 0) {
    strFilterValIndexes = `AND validator_id in [${validatorIndexes.map(i => `'${i}'`).join(',')}]`;
