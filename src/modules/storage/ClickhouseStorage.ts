...
import { ValStatus }                         from '../lighthouse/types/ValidatorStatus';
...
type NodeOperatorValSyncParticipationLessChainAvg = { nos_name: string, less_chain_avg_count: number };
type NodeOperatorValMissAttestation = { nos_name: string, miss_attestation_count: number };
type NodeOperatorValMissPropose = { nos_name: string, miss_propose_count: number };
...
export type ValidatorCounts = { active: number, pending: number, slashed: number }
...
export class ClickhouseStorage {
  ...
  public constructor(
    ...
    this.maxRetries = this.environment.DB_MAX_RETRIES;
    this.minBackoff = this.environment.DB_MIN_BACKOFF_SEC;
    this.maxBackoff = this.environment.DB_MAX_BACKOFF_SEC;
    this.chunkSize = this.environment.DB_INSERT_CHUNK_SIZE;

    this.logger.info('DB backoff set to (min=[%d], max=[%d] seconds) ', this.minBackoff, this.maxBackoff);
    this.logger.info('DB max retries set to [%d]', this.maxRetries);

    this.retry = retrier(this.logger, this.maxRetries, this.minBackoff * 1000, this.maxBackoff * 1000, true);

    this.db = new ClickHouse({
      url: this.environment.DB_HOST,
  ...
  public async getMaxSlot(): Promise<bigint> {
    const data: any = await this.retry(async () =>
      await this.db.query('SELECT max(slot) as max FROM stats.validator_balances').toPromise()
    );
  ...
  public async getMinSlot(): Promise<bigint> {
    const data: any = await this.retry(async () =>
      await this.db.query('SELECT min(slot) as min FROM stats.validator_balances').toPromise()
    );
  ...
  public async writeBalances(
    slot: bigint, slotTime: bigint, balances: StateValidatorResponse[], keysIndexed: KeysIndexed
    ...
    return await this.prometheus.trackTask('write-balances', async () => {
      ...
      this.logger.info(`Wrote Lido ${lidoCount} balances to DB and counted others`);
      return otherCounts;
    });
  }
  ...
  public async writeAttestations(
    ...
    slotTime: bigint, keysIndexed: KeysIndexed
  ): Promise<void> {
          ...
          for (const [block, blockAttestations] of Object.entries(attDutyResult.blocksAttestations)) {
            ...
            if (a.attested) {
              ...
              if (
                ...
                BigInt(this.environment.ATTESTATION_MAX_INCLUSION_IN_BLOCK_DELAY)
              ) a.attested = false;
              break;
  ...
  public async writeProposes(
    proposesDutiesResult: ProposerDutyInfo[], slotTime: bigint, keysIndexed: KeysIndexed
  ): Promise<void> {
  ...
  public async writeSyncs(
    ...
    return await this.prometheus.trackTask('write-syncs', async () => {
      ...
      const last_slot_of_epoch = epoch * BigInt(this.environment.FETCH_INTERVAL_SLOTS) + BigInt(this.environment.FETCH_INTERVAL_SLOTS) - 1n;
  ...
  public async getValidatorBalancesDelta(slot: bigint): Promise<NodeOperatorDelta[]> {
    const ret = await this.retry(async () => await this.db.query(
      validatorBalancesDeltaQuery(this.environment.FETCH_INTERVAL_SLOTS, slot.toString())).toPromise()
    );
    return <NodeOperatorDelta[]>ret;
  }

  public async getValidatorQuantile0001BalanceDeltas(slot: bigint): Promise<NodeOperatorDelta[]> {
    const ret = await this.retry(async () => this.db.query(
      validatorQuantile0001BalanceDeltasQuery(this.environment.FETCH_INTERVAL_SLOTS, slot.toString())
    ).toPromise());
    return <NodeOperatorDelta[]>ret;
  }
  ...
  public async getValidatorsCountWithNegativeDelta(slot: bigint): Promise<NodeOperatorNegativeDelta[]> {
    const ret = await this.retry(async () => this.db.query(
      validatorsCountWithNegativeDeltaQuery(this.environment.FETCH_INTERVAL_SLOTS, slot.toString())
    ).toPromise());
  ...
  public async getValidatorsCountWithSyncParticipationLessChainAvgLastNEpoch(
    slot: bigint, epochInterval: number, validatorIndexes: string[] = []
  ): Promise<NodeOperatorValSyncParticipationLessChainAvg[]> {
    const ret = await this.retry(async () => this.db.query(
      validatorsCountWithSyncParticipationLessChainAvgLastNEpochQuery(
        slot,
        this.environment.FETCH_INTERVAL_SLOTS,
        epochInterval,
        this.environment.SYNC_PARTICIPATION_DISTANCE_DOWN_FROM_CHAIN_AVG,
        validatorIndexes
      )
    ).toPromise());
  ...
  public async getValidatorsCountWithMissedProposes(
    slot: bigint, validatorIndexes: string[] = []
  ): Promise<NodeOperatorValMissPropose[]> {
    const ret = await this.retry(async () => this.db.query(
      validatorsCountWithMissProposeQuery(this.environment.FETCH_INTERVAL_SLOTS, slot.toString(), validatorIndexes)
    ).toPromise());
  ...
  public async getTotalBalance24hDifference(slot: bigint): Promise<number | undefined> {
    ...
    const {
      ...
      prev_total_balance,
      total_diff
    } = (<{ curr_total_balance: number, prev_total_balance: number, total_diff: number }>ret[0]);
